...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_formstatus` AS select distinct `cms_dg`.`dg_pid` AS `pid`,`cms_dg`.`dg_date` AS `dg_date`,(case when (`cms_apm`.`apm_pid` is not null) then 1 else 0 end) AS `apm_pid`,(case when (`cms_bsr`.`bsr_pid` is not null) then 1 else 0 end) AS `bsr_pid`,(case when (`cms_ce`.`ce_pid` is not null) then 1 else 0 end) AS `ce_pid`,(case when (`cms_cml`.`cml_pid` is not null) then 1 else 0 end) AS `cml_pid`,(case when (`cms_cprca`.`cprca_pid` is not null) then 1 else 0 end) AS `cprca_pid`,(case when (`cms_fupm`.`fupm_pid` is not null) then 1 else 0 end) AS `fupm_pid`,(case when (`cms_mdhl`.`mdhl_pid` is not null) then 1 else 0 end) AS `mdhl_pid`,(case when (`cms_vital`.`vital_pid` is not null) then 1 else 0 end) AS `vital_pid` from ((((((((`cms_dg` left join `cms_apm` on((`cms_dg`.`dg_pid` = `cms_apm`.`apm_pid`))) left join `cms_bsr` on((`cms_dg`.`dg_pid` = `cms_bsr`.`bsr_pid`))) left join `cms_ce` on((`cms_dg`.`dg_pid` = `cms_ce`.`ce_pid`))) left join `cms_cml` on((`cms_dg`.`dg_pid` = `cms_cml`.`cml_pid`))) left join `cms_cprca` on((`cms_dg`.`dg_pid` = `cms_cprca`.`cprca_pid`))) left join `cms_fupm` on((`cms_dg`.`dg_pid` = `cms_fupm`.`fupm_pid`))) left join `cms_mdhl` on((`cms_dg`.`dg_pid` = `cms_mdhl`.`mdhl_pid`))) left join `cms_vital` on((`cms_dg`.`dg_pid` = `cms_vital`.`vital_pid`))) group by `cms_dg`.`dg_pid` order by `cms_dg`.`dg_pid` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
