			...
			<div class="row">
				<div class="col-lg-12">
					...
					<?php if($model->isNewRecord) : ?>		
						   <?= $form->field($model, 'state_code', $template)->dropDownList($State,['options'=>[$Locmap =>['Selected'=>'selected']],'prompt' =>'Select']) ?>	
					<?php else: ?>
						   <?= $form->field($model, 'state_code', $template)->textInput(['value'=>$model->state_code,'readonly' => true]) ?>
					<?php endif; ?>	
					
					<?php if($model->isNewRecord) : ?>	
					   <?= $form->field($model, 'loc_code', $template)->textInput(['maxlength' => true]) ?>
					<?php else: ?>						
					    <?= $form->field($model, 'loc_code', $template)->textInput(['value'=>$model->loc_code,'readonly' => true]) ?>
					<?php endif; ?>	
										
					<?= $form->field($model, 'loc_name', $template)->textInput(['maxlength' => true]) ?>
