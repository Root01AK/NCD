		...
		<div class="panel-body">
			<div class="row">
				<div class="col-lg-12">
					...
					?>
					<?= $form->field($model, 'min_mnu_id_fk', $template)->dropDownList($mainmenus, ['prompt' => $model->getAttributeLabel("min_mnu_id_fk")]) ?>
					<?php if($model->isNewRecord): ?>
						<?= $form->field($model, 'sub_mnu_name', $template)->textInput(['maxlength' => true]) ?>
					<?php else: ?>
						<?= $form->field($model, 'sub_mnu_name', $template)->textInput(['maxlength' => true, 'disabled' => true, 'readonly' => true]) ?>
					<?php endif; ?>
					<?= $form->field($model, 'sub_mnu_desc', $template)->textInput(['maxlength' => true]) ?>
					<?= $form->field($model, 'sub_mnu_preference', $template)->textInput() ?>
