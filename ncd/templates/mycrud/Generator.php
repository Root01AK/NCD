    ...
    public function generateActiveField($attribute)
    {
        ...
        if ($tableSchema === false || !isset($tableSchema->columns[$attribute])) {
            if (preg_match('/^(password|pass|passwd|passcode)$/i', $attribute)) {
                return "\$form->field(\$model, '$attribute', \$template)->passwordInput()";
            } else {
                return "\$form->field(\$model, '$attribute', \$template)";
            }
        }
        $column = $tableSchema->columns[$attribute];
        if ($column->phpType === 'boolean') {
            return "\$form->field(\$model, '$attribute', \$template)->checkbox()";
        } elseif ($column->type === 'text') {
            return "\$form->field(\$model, '$attribute', \$template)->textarea(['rows' => 6])";
        } else {
            ...
            if (is_array($column->enumValues) && count($column->enumValues) > 0) {
                ...
                return "\$form->field(\$model, '$attribute', \$template)->dropDownList("
                    . preg_replace("/\n\s*/", ' ', VarDumper::export($dropDownOptions)).", ['prompt' => ''])";
            } elseif ($column->phpType !== 'string' || $column->size === null) {
                return "\$form->field(\$model, '$attribute', \$template)->$input()";
            } else {
                return "\$form->field(\$model, '$attribute', \$template)->$input(['maxlength' => true])";
            }
        }
    }
